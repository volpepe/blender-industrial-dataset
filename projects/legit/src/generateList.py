...
with open(os.path.sep.join(args.folder.split(os.path.sep)[:-1], 'custom_train_rawframes.txt'), 'w') as trainList:
    for vid in train:
...
with open(os.path.sep.join(args.folder.split(os.path.sep)[:-1], 'custom_val_rawframes.txt'), 'w') as valList:
    for vid in val:
        valList.write("{} {} {}\n".format(os.path.join(vid.split(os.path.sep)[-2], 
            vid.split(os.path.sep)[-1]),
            len(os.listdir(vid)),
